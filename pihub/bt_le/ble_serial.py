...
class BleSerial:
    ...
    def _handle_status_line(self, line: str) -> None:
        ...
        # Notify flags (keep as a dict, easy to extend)
        ...
        # Connection params: accept either *_ms (ints) or interval_ms (float)
        # Firmware currently prints interval_ms as float (e.g. 15.00) in your STATUS line.
        conn_params: Dict[str, Any] = {}
        if "interval_ms_x100" in kv:
            conn_params["interval_ms_x100"] = self._to_int(kv.get("interval_ms_x100"))
        if "interval_ms" in kv:
            # store x100 int to match your existing controller formatting
        ...
        self.state.conn_params = conn_params if conn_params else self.state.conn_params

        # PHY snapshot (your firmware prints phy_tx / phy_rx as ints)
        phy: Dict[str, Any] = {}
        if "phy_tx" in kv:
            phy["tx"] = self._to_int(kv.get("phy_tx"))
        if "phy_rx" in kv:
            phy["rx"] = self._to_int(kv.get("phy_rx"))
        self.state.phy = phy if phy else self.state.phy
